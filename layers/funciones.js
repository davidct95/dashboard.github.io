function obtenerColorZonas(
  ...
  if (distrito == "Rimac") {
    if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[0]
    ) {
      return arrayColoresObjeto[0];
    } else if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[1]
    ) {
      return arrayColoresObjeto[1];
    } else if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[2]
    ) {
      return arrayColoresObjeto[2];
    } else if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[3]
    ) {
      return arrayColoresObjeto[3];
    } else if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[4]
    ) {
      return arrayColoresObjeto[4];
    } else if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[5]
    ) {
      return arrayColoresObjeto[5];
    } else {
      return arrayColoresObjeto[6];
    }
    ...
    if (
      ...
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[6]
    ) {
      return arrayColoresObjeto[6];
    } else if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[7]
    ) {
      return arrayColoresObjeto[7];
    } else if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[8]
    ) {
      return arrayColoresObjeto[8];
    } else if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[9]
    ) {
      return arrayColoresObjeto[9];
    } else if (
      properties["afec_" + anioSeleccionado] === 1 &&
      properties.zona == zonasOrdenada[10]
    ) {
      return arrayColoresObjeto[10];
    } else {
      return arrayColoresObjeto[11];
    }
  }
}
...
function totalPorZona(anioSeleccionado, zonas, campo, distrito) {
  ...
  for (let i = 0; i < cantidad; i++) {
    for (const lote of capaDistrito.features) {
      if (
        lote.properties["afec_" + anioSeleccionado] > 0 &&
        lote.properties.zona == zonas[i]
      ) {
...
function ordenarArrays(array, arrayCantidad) {
  ...
  var zonasConCantidades = array.map((zona, index) => ({
    zona,
...
function updateCuadros(anioSeleccionado, distrito) {
  ...
  var imagenHospital =
    '<img src="img/iconos/edificio-del-hospital (1).png" alt="Descripción de la imagen" style="width: 50px;">&nbsp';

  var imagenColegio =
    '<img src="img/iconos/escuela.png" alt="Descripción de la imagen" style="width: 50px;">&nbsp';

  cuadro01.innerHTML = totalPersonasAfectadas(anioSeleccionado, distrito);
  cuadro03.innerHTML =
    imagenHospital + totalAfectados("hosp_", anioSeleccionado, distrito);
  cuadro04.innerHTML =
    imagenColegio + totalAfectados("cole_", anioSeleccionado, distrito);
}
...
function mostrarContenido(numeroPestana, anioSeleccionado, distrito) {
  ...
  var imagenLote =
    '<img src="img/iconos/lote.png" alt="Lote" style="width: 50px;">&nbsp';
  ...
  if (numeroPestana == 1) {
    var cuadro12 = document.getElementById("informacion1");
    cuadro12.innerHTML = imagenLote + totalAfectados("afec_", anioSeleccionado, dist);
    ...
    cantidadTotalZona = totalPorZona(
      anioSeleccionado,
      zonas,
      "afec_" + anioSeleccionado,
      dist
    );
...
function leyenda(legend, zonasOrdenada, distrito) {
  ...
  legend.onAdd = function (map) {
    ...
    div.innerHTML +=
      '<br><p style="font-weight: bold;">Instituciones</p><div class="cajita"><img src="' +
      colorIcono +
      '" class="icono"><p>Escuela</p></div>' +
...
function agregarIconos(anioSeleccionado, distrito, iconosColegio) {
  var dist = distrito;
  var colegios;
  var hospitales;
  ...
  iconosColegio = L.geoJson(colegios, {
    pointToLayer: function (feature, latlng) {
      return L.marker(latlng, {
        icon:
          feature.properties["colafe" + anioSeleccionado] == 1
            ? iconColegio
            : iconTransparente,
      });
    },
  ...
  var iconosHospital = L.geoJson(hospitales, {
    pointToLayer: function (feature, latlng) {
      return L.marker(latlng, {
        icon:
          feature.properties["hosafe" + anioSeleccionado] == 1
            ? iconHospital
            : iconTransparente,
      });
    },
  ...
  var data = [
    {
      type: "bar",
      x: [
        cantidadCallesAfectadas2017,
        cantidadCallesAfectadas1998,
        cantidadCallesAfectadas1987,
      ],
...
//Funcion para hacer click en lotes teselas y mostrar informacion

function onMapClick(e) {
  // Verifica si las coordenadas del clic están dentro de algún polígono
  var isInside = leafletPip.pointInLayer(e.latlng, areaEspecifica);
  if (isInside.length > 0) {
    // Obtiene el primer polígono dentro del cual se hizo clic
    var clickedPolygon = isInside[0];

    if (polygon !== null) {
      map.removeLayer(polygon);
    }

    var polygonCoords = clickedPolygon.toGeoJSON().geometry.coordinates;
    // Imprime las coordenadas del polígono
    console.log("Coordenadas del polígono:", polygonCoords[0][0]);

    for (let i = 0; i < polygonCoords[0][0].length; i++) {
      let point = polygonCoords[0][0][i]; // Accede a cada punto individual
      // Intercambiar los valores de posición
      let temp = point[0];
      point[0] = point[1];
      point[1] = temp;
    }
    ...
    polygon = L.polygon([polygonCoords[0][0]]).addTo(map);

    // Verifica si el polígono tiene propiedades definidas
    if (clickedPolygon.feature && clickedPolygon.feature.properties) {
      // Obtiene el valor de T_TOTAL del polígono
      var tTotalValue = clickedPolygon.feature.properties.T_TOTAL;
      // Imprime el valor de T_TOTAL del polígono
      console.log("T_TOTAL del polígono:", tTotalValue);

      // Actualiza el control informativo con el valor de T_TOTAL
      info.update({
        zona: clickedPolygon.feature.properties.zona,
        T_TOTAL: tTotalValue,
      });

      // Cambia el color del borde del polígono
    }
  }
}
